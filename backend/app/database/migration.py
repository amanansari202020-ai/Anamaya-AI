...
def run_migrations():
    ...
    try:
        ...
        db = SessionLocal()
        try:
            # Ensure facilities table has 12 facilities
            count = db.query(HealthcareFacility).count()
            if count < 12:
                logger.info("Seeding / updating 12 healthcare facilities in database...")

                # Define 12 realistic facilities (8 Government, 4 Private)
                facilities_data = [
                    {
                ...
                db.commit()
                logger.info("Successfully seeded facilities and doctors into database.")
            else:
                # Table has facilities, ensure ownership_type and doctors exist
                facilities = db.query(HealthcareFacility).all()
                for fac in facilities:
                    if not fac.ownership_type:
                        fac.ownership_type = "private" if not fac.is_government else "government"
                    
                    doc_count = db.query(Doctor).filter(Doctor.facility_id == fac.id).count()
                    if doc_count == 0:
                        # Seed 2 default doctors for existing facility
                        d1 = Doctor(
                            facility_id=fac.id,
                            name=f"Dr. Ramesh Patil",
                            degree="MBBS, MD",
                            specialization="General Physician",
                            years_experience=10,
                            available_days=["Mon", "Wed", "Fri"],
                            available_hours="09:00 AM - 01:00 PM"
                        )
                        d2 = Doctor(
                            facility_id=fac.id,
                            name=f"Dr. Sunita Deshmukh",
                            degree="MBBS, DGO",
                            specialization="Gynecologist",
                            years_experience=8,
                            available_days=["Tue", "Thu", "Sat"],
                            available_hours="10:00 AM - 02:00 PM"
                        )
                        db.add_all([d1, d2])
                db.commit()
                logger.info("Updated existing facilities with ownership_type and doctors.")
